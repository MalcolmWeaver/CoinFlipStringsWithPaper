...
def create_close_arr(g_arr, testalphabet):
    close_arr = []
    for i in range(len(g_arr)):
        for j in testalphabet:
            if j != :
            close_arr.append(g_arr[i*len(testalphabet) + j][:-1])
            close_arr[i*len(testalphabet) + j] += j
            else:
...
def main():
    teststr = input("This machine calculates the expected number of flips "
                    "before you first see a particular series of Flips/Rolls.")

    testalphabet = input("What are all possible roll/flip results (case "
                         "sensitive")
    g_arr = create_good_arr(teststr)
    c_arr = create_close_arr(g_arr, testalphabet)
